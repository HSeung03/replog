...
| 技術 | 説明 |
|------|------|
| GitHub Actions | PRごとにバックエンドFeatureテストを自動実行、モバイル依存関係の検査 |
| EAS Build | クラウドAndroid APKビルド |
| Railway | GitHub pushを検知して自動再デプロイ |
...
replog/
├── .github/workflows/
│   ├── backend-ci.yml        Laravel Featureテスト自動化
│   └── mobile-ci.yml         Expo依存関係・設定検査
